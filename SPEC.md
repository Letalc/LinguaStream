...
[Laptop de la sala] ──mic/mixer──▶ Página "Consola" (AudioWorklet → PCM 16kHz)
   ...
   Gemini Live API ──▶ transcripción original + traducción ES (streaming)
     ...
     Convex (conferences, sessions, segments, glossary, presence, heartbeats)
        │  queries reactivas
        ├──▶ /s/[sessionId]            Vista audiencia (elige sesión + idioma)
        ├──▶ /overlay/[id]?lang=es     Overlay transparente para OBS (Browser Source)
        ├──▶ /present/[id]             Presentación/pantalla con CC superpuesto
...
- Soporta **al menos 3 sesiones simultáneas** en la demo. La implementación usa 1 conexión Gemini por idioma de salida; los límites reales también dependen de las cuotas y del plan.
- Solo producción edita. Para la demo alcanza con una contraseña de admin por variable de entorno, sin sistema de usuarios.
- Open source: licencia MIT, README en español e inglés, `.env.example` y `docker-compose.yml`.
...
- [ ] 3 sesiones en paralelo, en 3 pestañas o 3 laptops, funcionan a la vez sin mezclarse. *(La lógica pasó con 3 y 10 salas offline; falta proveedor real.)*
- [ ] Una sesión de 30 min corre sin cortes visibles, con reconexión automática probada forzando una desconexión. *(Pasó con transporte simulado y tiempo virtual; falta reloj y red reales.)*
...
- [x] Panel `/admin` con estado, latencia, espectadores web y errores de cada sesión. *(Implementado; build y pruebas locales pasan.)*
