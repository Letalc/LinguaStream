...
/** `initialSession` comes from the server render, so the join popup shows without waiting for the websocket. */
export function AudienceView({ id, initialSession }: { id: string; initialSession?: Doc<"sessions"> | null }) {
  const sessionId = id as Id<"sessions">;
  const live = useQuery(api.sessions.get, { sessionId });
  const session = live === undefined ? initialSession : live;
